...
<template>
  <div class="pl-4 pr-4">
    <div class="mb-2">
      <span class="text-[13px] lg:text-[14px]">共 <b class="text-amber-700 text-[14px] lg:text-base">{{ count }}</b> 件商品</span>
    </div>
      ...
      <div class="flex items-center justify-between border-b border-gray-100 pt-2" v-for="(item, index) in cartList" :key="index" >
       ...
       <div :class="['pb-2', { 'flex items-center text-left': hIndex === 0 }]">
            ...
            <div :class="['text-[12px]', { 'pl-2 overflow-hidden text-ellipsis whitespace-nowrap': hIndex === 0 }]">
              ...
              <p v-if="hIndex === 4">-0</p>
              <p v-if="hIndex === 5">{{Number(item.cart_info.attrInfo.price * item.cart_info.cart_num).toFixed(2)}}</p>
            </div>
          </div>
        </div>
      </div>
