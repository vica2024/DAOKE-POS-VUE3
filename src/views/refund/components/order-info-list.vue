...
const headers = [
  {
    name: "refund.table.goodsInfo",
    width: 40,
    align: "left",
  },
  {
    name: "refund.table.num",
    align: "center",
    width: 8,
  },
  {
    name: "refund.table.weight",
    align: "center",
    width: 8,
  },
  {
    name: "refund.table.amount",
    width: 10,
    align: "center",
  },
  {
    name: "refund.table.discount",
    align: "center",
  },
  {
    name: "refund.table.total",
    width: 12,
    align: "center"
  },
];
...
<template>
  <div class="pl-4 pr-4">
    <div class="mb-2">
      <span class="text-[13px] lg:text-[14px]">{{ $t('refund.count') }} <b class="text-amber-700 text-[14px] lg:text-base">{{ cartList.length }}</b> {{ $t('refund.pieces') }}</span>
    </div>
       ...
       <div :class="['pb-2', { 'flex items-center text-left': hIndex === 0 }]">
            ...
            <div :class="['text-[12px]', { 'pl-2 overflow-hidden text-ellipsis whitespace-nowrap': hIndex === 0 }]">
              <p class="mt-1" v-if="hIndex === 0">
                <h3>{{ item.productInfo.store_name }}</h3>
                <a-tag class="text-[10px]" size="small">{{ item.attrInfo.suk }}</a-tag>
              </p>
              <p v-if="hIndex === 1">{{item.cart_num}}</p>
              <p v-if="hIndex === 2">{{item.attrInfo.weight}}</p>
              <p v-if="hIndex === 3">{{item.attrInfo.price}}</p>
              <p v-if="hIndex === 4">-0</p>
              <p v-if="hIndex === 5">{{Number(item.attrInfo.price * item.cart_num).toFixed(2)}}</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <a-empty v-else class="mt-[150px]"/>
      ...
      <div  class="h-[160px] flex justify-between text-sm mt-4 rounded-lg overflow-hidden">
        ...
        <p> {{$t('refund.refundAmount')}} : ¥<b class="text-base">-{{ refund_price }}</b></p>
    </div>
      </div>
  </div>
</template>
